...
class DCCF1(GeneralModel):
    ...
    def parse_model_args(parser):
        ...
        parser.add_argument('--n_intents', type=int, default=64, # [优化] 默认意图数增加到64
                            help='Number of latent intents (K).')
        parser.add_argument('--ssl_temp', type=float, default=0.1,
                            help='Temperature for contrastive loss.')
        parser.add_argument('--ssl_reg', type=float, default=0.1, # [优化] 默认SSL权重增加，强化稀疏数据下的信号
                            help='Weight for contrastive loss.')
        parser.add_argument('--cen_reg', type=float, default=0.0, # [优化] 暂时关闭中心正则，简化优化目标
                            help='Weight for intent centering/independence loss.')
        parser.add_argument('--emb_reg', type=float, default=1e-4, # [优化] 降低L2正则，防止欠拟合
                            help='Weight for L2 regularization.')
        parser.add_argument('--mask_update_interval', type=int, default=20, 
                            help='[New] Update adaptive mask every N steps to speed up CPU training.')
    ...
    def __init__(self, args, corpus):
        ...
        self.mask_update_interval = args.mask_update_interval
        ...
        # [新增] 缓存机制
        self.forward_step = 0
        self.mask_cache = {} # Key: "layer_idx_type", Value: (indices, values)
    ...
    def _adaptive_mask(self, all_embeddings, h_list, t_list):
        ...
        # [优化] 移除烦人的 DEBUG print，只保留 NaN 警告
    ...
    def _graph_propagation(self):
        ...
        # [新增] 判断是否需要更新 Mask
        # 只有在训练模式下，且步数满足间隔时才更新。否则使用缓存。
        update_mask = self.training and (self.forward_step % self.mask_update_interval == 0)
        ...
        for i in range(self.n_layers):
            ...
            # === Adaptive Mask Logic with Caching ===
            graph_key = f"{i}_graph"
            inten_key = f"{i}_inten"

            # 1. 计算或读取 Mask (G_graph)
            if update_mask or (graph_key not in self.mask_cache):
                G_graph_indices, G_graph_values = self._adaptive_mask(gnn_layer_embeddings, self.all_h_list, self.all_t_list)
                self.mask_cache[graph_key] = (G_graph_indices, G_graph_values)
                gc.collect()
            else:
                G_graph_indices, G_graph_values = self.mask_cache[graph_key]

            # 2. 计算或读取 Mask (G_inten)
            if update_mask or (inten_key not in self.mask_cache):
                G_inten_indices, G_inten_values = self._adaptive_mask(int_layer_embeddings, self.all_h_list, self.all_t_list)
                self.mask_cache[inten_key] = (G_inten_indices, G_inten_values)
                gc.collect()
            else:
                G_inten_indices, G_inten_values = self.mask_cache[inten_key]
            # ========================================
    ...
    def inference(self, feed_dict):
        ...
        # [优化] 评估时，只调用一次传播，不再重新计算Mask（复用训练时的缓存）
